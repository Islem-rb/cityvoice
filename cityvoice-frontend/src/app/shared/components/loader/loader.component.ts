...
export class LoaderComponent implements AfterViewInit, OnDestroy {
  ...
  getTicks(): { x1: number; y1: number; x2: number; y2: number; major: boolean }[] { const t: { x1: number; y1: number; x2: number; y2: number; major: boolean }[] = []; for(let i=0;i<60;i++){const a=(i/60)*Math.PI*2-Math.PI/2,l=i%5===0?8:4,r1=98,r2=r1-l;t.push({x1:+(100+Math.cos(a)*r1).toFixed(2),y1:+(100+Math.sin(a)*r1).toFixed(2),x2:+(100+Math.cos(a)*r2).toFixed(2),y2:+(100+Math.sin(a)*r2).toFixed(2),major:i%5===0});} return t; }
  ...
  private buildScene(): void {
    ...
    this.buildStreets();
    const count = Math.min(150, Math.floor(this.W * this.H / 12000)); // réduit de 600 → 150
    ...
    const colCount = Math.min(8, Math.floor(this.W / 150));
  ...
  private drawConnections(ctx: CanvasRenderingContext2D): void {
    ...
    ctx.save();
    // Un seul beginPath + stroke pour TOUTES les connexions — évite 11 250 stroke() par frame
    ctx.globalAlpha = this.connectionAlpha * 0.12;
    ctx.strokeStyle = this.TEAL;
    ctx.lineWidth   = 0.5;
    ctx.beginPath();
    // Limite à 800 connexions max pour éviter le freeze
    const limit = Math.min(this.connections.length, 800);
    for (let k = 0; k < limit; k++) {
      const [i, j] = this.connections[k];
