...
export class AuthService {
  ...
  logout(): void {
    ...
    // Invalider la session OAuth2 côté Spring (sans bloquer si le serveur est down)
    this.http.post(
      'http://localhost:8081/api/auth/spring-logout', {},
  ...
  getUserId(): string {
    ...
    // Si l'utilisateur est connecté, retourne son userId, sinon chaîne vide
    return user?.userId ?? '';
  }
...
getNom(): string {
  ...
  try {
    const payload = JSON.parse(atob(token.split('.')[1]));
    return payload.nom || payload.name || '';
  } catch { return ''; }
}
