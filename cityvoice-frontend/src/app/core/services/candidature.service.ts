...
addCandidature(candidature: CandidatureEquipe, equipeId: string) {
  return this.http.post<CandidatureEquipe>(
    `${this.base}/add/${equipeId}`,
    candidature
  );
}
getByEquipeAndFonction(equipeId: string, fonction: Fonction): Observable<CandidatureEquipe | null> {
  return this.http.get<CandidatureEquipe>(
    `${this.base}/equipe/${equipeId}/fonction/${fonction}`
  );
}
