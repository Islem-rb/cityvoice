...
export class AdminDashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.heatCells = Array.from({ length: 182 }, () => Math.random());
    this.loadData();
  ...
  private loadData(): void {
    ...
    // ⚠️  getAll() désactivé temporairement — retourne les photos en base64 (LONGTEXT)
    // ce qui génère 50-200 MB de JSON et freeze le navigateur.
    // À réactiver après mvn clean package -DskipTests sur signalement-service.
    //
    // this.signalementSvc.getAll().subscribe({ next: (list) => { ... } });
  }
  ...
  private refreshMapMarkers(): void {
    ...
    const filtered = this.signalements
      .filter(s => {
        if (this.mapFilter === 'all') return true;
        return s.statut === filterMap[this.mapFilter];
      })
      .slice(0, 60); // Limite à 60 marqueurs max pour éviter le freeze
    ...
    filtered.forEach(sig => {
      ...
      // Pas d'animation CSS infinie — remplacée par un simple cercle statique
      const icon = L.divIcon({
        html: `<div style="position:relative;width:20px;height:20px">
          <div style="position:absolute;inset:-3px;border-radius:50%;border:1.5px solid ${color};opacity:.4"></div>
