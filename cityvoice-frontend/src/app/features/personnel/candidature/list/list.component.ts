...
export class ListComponent implements OnInit {
  ...
  // ========== IA Variables ==========
  showAiModal = false;
  selectedCandidatureForAi: CandidatureEquipe | null = null;
  isGeneratingPdf = false;
  generatingForId: string | null = null;
  // ==================================
  ...
  genererCvIa(c: CandidatureEquipe): void {
    if (!c.id) return;
    const userId = this.authService.getUserId();
    if (!userId) {
      alert('Utilisateur non connecté');
    ...
    if (this.generatingForId) return;

    this.generatingForId = c.id;
    this.isGeneratingPdf = true;
    this.cvService.genererCvParIa(c.id, userId).subscribe({
      next: blob => {
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = `CV_${(c.equipe?.name || 'candidature').replace(/\s+/g, '_')}.pdf`;
        a.click();
        this.generatingForId = null;
...
// Dans ListComponent, ajoutez cette méthode
