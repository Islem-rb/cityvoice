...
export class ChefEquipeComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  this.equipeSvc.getAll().subscribe({
    next: equipes => {
      // ── Stratégie 1 : membre déjà lié via userId (comparaison normalisée) ──
      for (const eq of equipes) {
        const isMine = (eq.membresEquipe || []).some((m: any) => {
          if (!m.userId) return false;
          // Normaliser les deux IDs : retirer tirets et mettre en minuscules
          const mId    = m.userId.replace(/-/g, '').toLowerCase();
          const chefId = this.chefId.replace(/-/g, '').toLowerCase();
          return mId === chefId;
        });
  ...
  this.cd.detectChanges();
  // Recharger les signalements maintenant que les contrats sont connus
  this.loadSignalements();   // ← ajouter cette ligne ici
  if (!this.equipeCode) {
    const found = list.find(c => c.equipeCode);
    if (found?.equipeCode) {
      this.equipeCode  = found.equipeCode;
      this.equipeLabel = found.equipeLabel ?? '';
      localStorage.setItem('equipeCode',  this.equipeCode);
      localStorage.setItem('equipeLabel', this.equipeLabel);
    }
  }
};
  ...
  // IDs des signalements liés aux contrats du chef (acceptés ou en attente)
  ...
  const sub = this.sigSvc.getAll().subscribe({
    next: all => {
      this.signalements = all.filter((s: any) => {
        // Priorité 1 : le signalement est dans un contrat du chef
        if (contratSigIds.includes(s.id)) return true;
        // Priorité 2 : fallback par equipeIA
  ...
  accepterContratDetail(): void {
    ...
    // S'assurer que equipeCode est connu avant de charger les signalements
    if (!this.equipeCode && this.selectedContrat.equipeCode) {
      this.equipeCode  = this.selectedContrat.equipeCode;
      this.equipeLabel = this.selectedContrat.equipeLabel ?? '';
      localStorage.setItem('equipeCode',  this.equipeCode);
      localStorage.setItem('equipeLabel', this.equipeLabel);
    }
  ...
  accepterContrat(): void {
    ...
    // S'assurer que equipeCode est connu avant de charger les signalements
    if (!this.equipeCode && this.signatureMode.equipeCode) {
      this.equipeCode  = this.signatureMode.equipeCode;
      this.equipeLabel = this.signatureMode.equipeLabel ?? '';
      localStorage.setItem('equipeCode',  this.equipeCode);
      localStorage.setItem('equipeLabel', this.equipeLabel);
    }
    this.actionLoading = true;
